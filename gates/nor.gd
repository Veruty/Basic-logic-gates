...
func _draw() -> void:
	...
	ObjectMaterial.albedo_color = Color("#00fff7")
	GateMeshInstance.material_overlay = ObjectMaterial

func _gateBehaviour():
	if Pegs[0].State == Peg.States.ON or Pegs[1].State == Peg.States.ON:
		Pegs[2].State = Peg.States.OFF
	else:
		Pegs[2].State = Peg.States.ON
